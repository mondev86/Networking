        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
          <div class="p-6 text-gray-900">
            <form @submit.prevent="submitForm" class="space-y-6">
              ...
              <!-- ASIGNACIÓN OPCIONAL (solo si no tiene asignación activa) -->
<div v-if="!hasCurrentAssignment" class="border-t pt-6">
  <div class="flex items-center gap-3 mb-4">
    ...
    <label for="assign_now" class="text-sm font-medium text-gray-700 cursor-pointer">
      Asignar a un usuario/departamento/equipo
    </label>
  </div>
  ...
  <!-- Si ya tiene asignación, mostrar aviso -->
  <div v-if="hasCurrentAssignment" class="bg-blue-50 p-3 rounded-lg text-sm text-blue-700">
    ℹ️ Este dispositivo ya tiene una asignación activa. Para reasignar, ve a la página de detalle.
  </div>
</div>
...
// Ya tiene asignación activa?
const hasCurrentAssignment = computed(() => !!props.device.current_assignment);
...
const submitForm = (): void => {
  ...
  form.put(route('equipment.update', props.device.id), {
    onSuccess: () => {},
  });
};
</script>
